                            ...
                            try:
                                if line.find('MESSAGE	item_update') != -1:
                                    ...
                                    try:
                                        ...
                                        print 'error updating database'
                                        print e
                                elif line.find('MESSAGE event_notification  {\"type\":\"impression\"') != -1:
                                    n_impressions = n_impressions + 1
                                    newstring = line.strip('MESSAGE event_notification')
                                    newstring = newstring.rsplit("}",1)[0] + '}'
                                    data = json.loads(newstring)
                                    try:
                                        ...
                                        print 'error updating database'
                                        print e
                                elif line.find('MESSAGE recommendation_request') != -1:
                                    n_requests = n_requests + 1
                                    newstring = line.strip('MESSAGE recommendation_request')
                                    newstring = newstring.rsplit("}",1)[0] + '}'
                                    data = json.loads(newstring)
                                    try:
                                        updateClickInMongo(collection, data)
